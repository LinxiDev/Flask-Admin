...
class LinOrders(BaseModel):
    __tablename__ = 'lin_orders'

    id = db.Column(db.BigInteger, primary_key=True, comment='ID')
    order_id = db.Column(db.String(100), nullable=False, comment='问题ID')
    money = db.Column(db.Float, nullable=False, comment='金额')
    status = db.Column(db.Integer, nullable=False, comment='状态')
    product = db.Column(db.String(255), nullable=False, comment='产品名称')
    time = db.Column(db.DateTime, nullable=False, default=datetime.now, comment='创建时间')
